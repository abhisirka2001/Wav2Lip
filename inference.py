...
parser.add_argument('--face_default',default="../images (1).jfif", type =str,
                    help='Prevent failing when no face detected')
...
def face_detect(images):
    ...
    for image, (rect,score) in zip(images, face_rect(images)):
        if len(rect)==0 :
            

            results.append([image, [0,0,0,0]])
...
def datagen(frames, mels):
    ...
    for i, m in enumerate(mels):
        ...
        # Check the value and dimensions of face

        # Check if the face region is empty
        if len(coords) and face.size  == 0:
            face = np.random.rand(args.img_size, args.img_size, 3) * 255
            coords_batch.append([])
            # Handle the case where the face region is empty
            print("Skipping resizing as face region is empty")
        else: 
...
def main():
    ...
    os.makedirs("frames_with_indices", exist_ok=True)
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
        ...
        for p, f, c, idx in zip(pred, frames, coords, range(len(frames))):
            if len(c) == 0:  # Check if there are no face coordinates (no face detected)
                ...
                f[y1:y2, x1:x2] = p
                out.write(f)  # Write the lip-synced frame to the output videon the 'frames' directory
        ...
        for faces in all_faces:
            if faces:
                for face in faces:
                    ...
                    yield tuple(map(int, box)), score
            else:
                # If no faces are detected, yield a placeholder value
                yield (0, 0, 0, 0), 0
                
def face_rect_single(image):
    faces = detector([image])  # return faces list of the image
    if faces:
        try:
            box, landmarks, score = faces[0]
            return tuple(map(int, box))
        except (ValueError, TypeError):
            # If unpacking fails, return the entire image as the face region
